...
export default function AdminOrdersPage() {
  ...
  const exportRows = rows.map((o) => ({
    ...
    itemsCount: o.itemCount ?? (o.items ?? []).length,
        ...
        columns={[
          {
            ...
            header: 'Order',
            cell: (r) => (
              <Link className="font-medium hover:underline" to={`/admin/orders/${r.orderId}`}>
                {r.orderId}
              </Link>
            ),
          },
          ...
          },
          {
            ...
            header: 'Products',
            cell: (r) => (
              <span className="text-muted">{r.itemCount ?? (r.items ?? []).length}</span>
            ),
          },
          {
            key: 'total',
            header: 'Total',
            cell: (r) => <span className="font-medium">{formatCurrency(r.totalAmount ?? 0)}</span>,
          },
          {
            key: 'date',
            header: 'Date',
            cell: (r) => <span className="text-muted">{formatDateTime(r.datetime)}</span>,
          },
          {
            key: 'status',
            header: 'Status',
            cell: (r) => (
              <div className="flex items-center gap-2">
                ...
                <Select
                  value={r.status}
                  onValueChange={(v) =>
                    update.mutate({ id: r.orderId, status: v as OrderStatus })
                  }
                >
