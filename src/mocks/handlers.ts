...
// Helper: returns a fully-joined order object (same shape as GET /orders/:id)
function joinedOrder(orderId: string) {
...
export const handlers = [
  ...
  // FIX: was returning bare db.orders[idx] without customer/admin/items joined —
  // the detail page would lose its items after an assignAdmin mutation.
  http.put('/orders/:id', async ({ params, request }) => {
    ...
    return json(joinedOrder(id)!)
  }),

  // FIX: was returning bare db.orders[idx] without customer/admin/items joined —
  // after a status change the cache would be updated with an order missing items,
  // causing the detail page to go blank and the customer order page to show no items.
